...
import LimitedUseItem, { ILimitedUseItem, ILimitedUseItemConfig } from "../LimitedUseItem";
...
export default class UpgradeItem extends LimitedUseItem implements IUpgradeItem {
    private _startingValue: number
    private _priceProgression: number[]

    get price() {
        if (this._priceProgression[this.quantity]) return this._priceProgression[this.quantity]
        return super.price
    }
    ...
    protected apply(game: GameController): void {
        game.updateSettings({ [this._id]: this.nextValue })
    }
