...
describe("Board.ts", () => {
  ...
  test("must get square valid movement correctly", () => {
    ...
    // Free moves
    expect(board.getSquareValidMovement(3, 3, MoveDirection.Up)).toEqual([0, 3]);
    expect(board.getSquareValidMovement(0, 0, MoveDirection.Right)).toEqual([0, 3]);
    expect(board.getSquareValidMovement(0, 0, MoveDirection.Down)).toEqual([3, 0]);
    expect(board.getSquareValidMovement(3, 3, MoveDirection.Left)).toEqual([3, 0]);
    ...
    expect(board.getSquareValidMovement(2, 1, MoveDirection.Up)).toEqual([0, 1]);
    expect(board.getSquareValidMovement(2, 1, MoveDirection.Right)).toEqual([2, 3]);
    board.loadPreset([0, 0, 0, 0, 2, 0, 2, 0, 0, 0, 0, 0, 0, 0, 2, 0]);
    expect(board.getSquareValidMovement(1, 2, MoveDirection.Down)).toEqual([3, 2]);
    expect(board.getSquareValidMovement(1, 2, MoveDirection.Left)).toEqual([1, 0]);

    // won't merge
    board.loadPreset([0, 4, 0, 0, 0, 0, 0, 0, 0, 2, 0, 4, 0, 0, 0, 0]);
    expect(board.getSquareValidMovement(2, 1, MoveDirection.Up)).toEqual([1, 1]);
    expect(board.getSquareValidMovement(2, 1, MoveDirection.Right)).toEqual([2, 2]);
    board.loadPreset([0, 0, 0, 0, 4, 0, 2, 0, 0, 0, 0, 0, 0, 0, 4, 0]);
    expect(board.getSquareValidMovement(1, 2, MoveDirection.Down)).toEqual([2, 2]);
    expect(board.getSquareValidMovement(1, 2, MoveDirection.Left)).toEqual([1, 1]);
  });
